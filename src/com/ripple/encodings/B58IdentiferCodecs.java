...
public class B58IdentiferCodecs {
    ...
    public B58IdentiferCodecs(Base58 base58encoder) {
        this.base58 = base58encoder;
    }

    public byte[] decodeFamilySeed(String master_seed) {
        return base58.decodeChecked(master_seed, VER_FAMILY_SEED);
    }

    public String encodeFamilySeed(byte[] bytes) {
        return base58.encodeToStringChecked(bytes, VER_FAMILY_SEED);
    }

    public String encodeAddress(byte[] bytes) {
        return base58.encodeToStringChecked(bytes, VER_ACCOUNT_ID);
    }

    public byte[] decodeAddress(String address) {
        return base58.decodeChecked(address, VER_ACCOUNT_ID);
    }
}
