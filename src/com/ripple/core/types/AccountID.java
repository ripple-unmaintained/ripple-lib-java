...
public class AccountID implements SerializedType, Comparable<AccountID> {
    ...
    private static void populateFieldsFromKeyPair(AccountID a, KeyPair kp) {
        a.keyPair = kp;
        a.addressBytes = Utils.SHA256_RIPEMD160(kp.getPub().toByteArray());
    ...
    public static KeyPair keyPairFromSeedString(String master_seed) {
        return keyPairFromSeedBytes(getB58IdentiferCodecs().decodeFamilySeed(master_seed));
    }

    public static KeyPair keyPairFromSeedBytes(byte[] master_seed) {
    ...
    public abstract static class AccountIDField extends AccountID implements HasField {
    }
